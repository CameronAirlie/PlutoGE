...
const setPanelDragOccluded = (occluded: boolean): void =>
	window.plutoEditor.setViewportOccluded("panel-drag", occluded);
let nextDockId = 1;
const createDockId = (): string =>
	`dock-${Date.now().toString(36)}-${nextDockId++}`;
const tabs = (id: string, panels: PanelId[], active = panels[0]): DockTabs => ({
	type: "tabs",
	id,
	panels,
	active,
...
const cloneDefaultLayout = (): DockNode =>
...
const addPerformancePanel = (node: DockNode): DockNode => {
	...
	return {
		...node,
		first: addPerformancePanel(node.first),
		second: addPerformancePanel(node.second),
	};
};

const parseDockNode = (
	value: unknown,
	seenPanels: Set<PanelId>,
): DockNode | undefined => {
	...
	if (
		...
		const panels = node.panels.filter(
			(panel): panel is PanelId => isPanelId(panel) && !seenPanels.has(panel),
		);
		...
		return tabs(
			node.id,
			panels,
			isPanelId(node.active) && panels.includes(node.active)
				? node.active
				: panels[0],
		);
	}
...
const loadLayout = (): DockNode => {
	try {
		const parsed = parseDockNode(
			JSON.parse(localStorage.getItem("plutoge:dock-tree:v2") ?? ""),
			new Set(),
		);
		if (parsed) {
			if (localStorage.getItem(performancePanelMigrationKey) !== "1") {
				localStorage.setItem(performancePanelMigrationKey, "1");
				return addPerformancePanel(parsed);
			}
			return parsed;
		}
	} catch {
	...
	if (node.type === "tabs") {
		...
		return panels.length
			? {
					...node,
					panels,
					active: node.active === panel ? panels[0] : node.active,
				}
			: undefined;
	}
...
const updateTabs = (
	node: DockNode,
	id: string,
	update: (tabsNode: DockTabs) => DockNode,
): DockNode => {
	if (node.type === "tabs") return node.id === id ? update(node) : node;
	return {
		...node,
		first: updateTabs(node.first, id, update),
		second: updateTabs(node.second, id, update),
	};
};
...
const updateSplitRatio = (
	...
	return {
		...node,
		first: updateSplitRatio(node.first, id, ratio),
		second: updateSplitRatio(node.second, id, ratio),
	};
};

const findTabsWithPanel = (
	node: DockNode,
	panel: PanelId,
): string | undefined => {
	if (node.type === "tabs")
		return node.panels.includes(panel) ? node.id : undefined;
	return (
		findTabsWithPanel(node.first, panel) ??
		findTabsWithPanel(node.second, panel)
	);
};

const firstTabsId = (node: DockNode): string =>
	node.type === "tabs" ? node.id : firstTabsId(node.first);
...
const insertPanel = (
	...
	updateTabs(node, targetId, (target) => {
		if (position === "center")
			return { ...target, panels: [...target.panels, panel], active: panel };
...
const preferredTarget = (layout: DockNode, panel: PanelId): string => {
	const companion: Partial<Record<PanelId, PanelId>> = {
		game: "viewport",
		viewport: "game",
		hierarchy: "inspector",
		inspector: "hierarchy",
...
export function useDockLayout(): {
	...
	useEffect(() => {
		const timeout = window.setTimeout(
			() =>
				localStorage.setItem("plutoge:dock-tree:v2", JSON.stringify(layout)),
			150,
		);
...
function SplitHandle({
	...
	const drag = useRef<
		| { pointerId: number; start: number; ratio: number; size: number }
		| undefined
	>(undefined);
		...
		<div
			...
			onPointerDown={(event) => {
				...
				drag.current = {
					...
					ratio: node.ratio,
					size: horizontal ? parent.width : parent.height,
				};
			}}
			onPointerMove={(event) => {
				...
				onRatioChange(
					drag.current.ratio +
						(coordinate - drag.current.start) / Math.max(1, drag.current.size),
				);
			}}
			onPointerUp={(event) => {
			...
			onKeyDown={(event) => {
				...
				onRatioChange(node.ratio + (increase ? 0.03 : -0.03));
			}}
		/>
	);
}

function DockTabsView({
	node,
	panels,
	onActivate,
	onClose,
	...
	const positionForEvent = (event: React.DragEvent): DockDropPosition => {
		...
		if (y < 0.24) return "top";
		if (y > 0.76) return "bottom";
		return "center";
	};
	return (
		<section
		...
		>
			<div className="dock-tab-strip" role="tablist">
				{node.panels.map((panel) => (
					<div
						className={`dock-tab ${node.active === panel ? "active" : ""}`}
						key={panel}
					>
						<button
			...
			{dropPosition && (
				<div className={`dock-drop-preview ${dropPosition}`}>
					<span>
						{dropPosition === "center" ? "Add as tab" : `Split ${dropPosition}`}
					</span>
				</div>
			)}
		</section>
...
function DockNodeView({
	...
	return (
		<div className={`dock-split ${node.direction}`}>
			<div
			...
			<SplitHandle
				node={node}
				onRatioChange={(ratio) =>
					onLayoutChange(updateSplitRatio(node, node.id, ratio))
				}
			/>
			<div
				className="dock-split-child"
...
export function DockWorkspace({
	...
	gameHostPerformance?: HostPerformance;
	editor?: EditorState;
