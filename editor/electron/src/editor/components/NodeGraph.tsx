...
export function NodeGraph({
	...
	const [instance, setInstance] =
		React.useState<ReactFlowInstance<FlowNode, Edge>>();
	const [createMenu, setCreateMenu] = React.useState<{
		clientX: number;
		clientY: number;
		x: number;
		y: number;
		from?: { nodeId: number; pin: string };
	}>();
	...
	const [flowNodes, setFlowNodes, applyNodeChanges] =
		useNodesState<FlowNode>(incomingNodes);
		...
		<div className="node-graph">
			<ReactFlow<FlowNode, Edge>
				...
				onConnectEnd={(event, state) => {
					...
					setCreateMenu({
						clientX: state.pointer.x,
						clientY: state.pointer.y,
						x: position.x,
						y: position.y,
						from: {
							nodeId: Number(state.fromNode.id),
							pin: state.fromHandle.id ?? "Out",
						},
					});
				}}
				onDragOver={(event) => {
					if (event.dataTransfer.types.includes("application/x-plutoge-node")) {
						event.preventDefault();
						event.dataTransfer.dropEffect = "copy";
					}
				}}
				onDrop={(event) => {
					const kind = event.dataTransfer.getData("application/x-plutoge-node");
					...
					onCreateNode?.(kind, position.x, position.y);
				}}
				onNodesDelete={(deleted) =>
					deleted.forEach((node) => onDelete?.(Number(node.id)))
				}
				fitView
				minZoom={0.25}
				maxZoom={2.5}
			...
			<div className="node-graph-help">
				Drag between ports to connect · Wheel to zoom · Drag background to pan
			</div>
			{createMenu && (
				<DropdownMenu.Root
					open
					modal={false}
					onOpenChange={(open) => {
						if (!open) setCreateMenu(undefined);
					}}
				>
					...
					<DropdownMenu.Portal>
						<DropdownMenu.Content
							className="graph-create-menu"
							side="bottom"
							align="start"
							sideOffset={6}
							collisionPadding={8}
						>
							<DropdownMenu.Label className="graph-create-label">
								Create connected node
							</DropdownMenu.Label>
							{palette
								.filter((item) => item.connectable !== false)
								.map((item) => (
									<DropdownMenu.Item
										className="graph-create-item"
										key={item.kind}
										onSelect={() => {
											onCreateNode?.(
												item.kind,
