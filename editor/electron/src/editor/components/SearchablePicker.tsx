...
let nextPickerId = 1;
export interface SearchablePickerItem<T extends string = string> {
	value: T;
	label: string;
	category?: string;
	keywords?: string[];
...
export function SearchablePicker<T extends string>({
	...
	useLayoutEffect(() => {
		...
		window.plutoEditor.setViewportOccluded(token, true);
		return () => window.plutoEditor.setViewportOccluded(token, false);
	}, [open, token]);
	const close = (): void => {
		setOpen(false);
		setQuery("");
	...
	return (
		...
		>
			<Popover.Trigger asChild>
				<Button
					className={`searchable-picker-trigger ${className}`}
					disabled={disabled}
				>
			...
			<Popover.Portal>
				...
				>
					...
					/>
					<ScrollBox className="searchable-picker-results">
						...
						{!filtered.length && <small>{emptyMessage}</small>}
					</ScrollBox>
				</Popover.Content>
			</Popover.Portal>
		</Popover.Root>
