...
export function Toolbar({ editor, running, showEditorCamera, visiblePanels, onToggleCamera, onTogglePanel, onResetLayout }: {
  ...
  const closeMenu = (): void => { setMenu(undefined); setActiveMenu(''); };
  const openMenu = (event: React.MouseEvent<HTMLButtonElement>, name: string, items: PopupMenuItem[]): void => {
  ...
  const fileItems: PopupMenuItem[] = [
    { label: 'New Project…', action: () => void window.plutoEditor.newProject() },
    { label: 'Open Project…', action: () => void window.plutoEditor.openProject() },
    { label: 'Save Project', disabled: !editor?.projectPath, action: () => void window.plutoEditor.saveProject() },
    { label: 'New Scene', separatorBefore: true, action: () => void window.plutoEditor.newScene() },
    { label: 'Open Scene…', action: () => void window.plutoEditor.openScene() },
    { label: 'Save Scene', shortcut: 'Ctrl+S', action: () => void window.plutoEditor.saveScene() },
    { label: 'Save Scene As…', shortcut: 'Ctrl+Shift+S', action: () => void window.plutoEditor.saveScene(true) },
  ];
  const editItems: PopupMenuItem[] = [
    { label: 'Undo', shortcut: 'Ctrl+Z', disabled: !editor?.canUndo || running, action: () => window.plutoEditor.undo() },
    { label: 'Redo', shortcut: 'Ctrl+Y', disabled: !editor?.canRedo || running, action: () => window.plutoEditor.redo() },
    { label: 'Duplicate Selected', shortcut: 'Ctrl+D', disabled: !editor?.selectedEntityId || running, separatorBefore: true, action: () => editor?.selectedEntityId && window.plutoEditor.duplicateEntity(editor.selectedEntityId) },
    { label: 'Delete Selected', shortcut: 'Del', disabled: !editor?.selectedEntityId || running, danger: true, action: () => editor?.selectedEntityId && window.plutoEditor.deleteEntity(editor.selectedEntityId) },
  ];
  ...
  return <header className="toolbar">
    ...
    <nav className="toolbar-menubar" aria-label="Editor menu">
      <button className={activeMenu === 'file' ? 'active' : ''} onClick={(event) => openMenu(event, 'file', fileItems)}>File</button>
      <button className={activeMenu === 'edit' ? 'active' : ''} onClick={(event) => openMenu(event, 'edit', editItems)}>Edit</button>
      <button className={activeMenu === 'create' ? 'active' : ''} onClick={(event) => openMenu(event, 'create', createItems)}>Create</button>
      <button className={activeMenu === 'view' ? 'active' : ''} onClick={(event) => openMenu(event, 'view', viewItems)}>View</button>
    </nav>
    ...
    <PopupMenu menu={menu} onClose={closeMenu} className="toolbar-popup-menu" />
  </header>;
}
