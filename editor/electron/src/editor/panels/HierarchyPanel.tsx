...
function HierarchyNode({ entity, entities, selectedId, disabled, openContextMenu }: {
  entity: EditorEntity;
  ...
  return <div className="hierarchy-node">
    ...
    {children.length > 0 && <div className="tree-children">
      {children.map((child) => <HierarchyNode key={child.id} entity={child} entities={entities} selectedId={selectedId} disabled={disabled} openContextMenu={openContextMenu} />)}
    </div>}
  </div>;
}
...
export function HierarchyPanel({ editor, selectedEntity }: { editor?: EditorState; selectedEntity?: EditorEntity }): React.JSX.Element {
  ...
  const createItems = (parentId: number): PopupMenuItem[] => entityPresets.map((name) => ({
    label: name,
    disabled: running || !editor,
    action: () => window.plutoEditor.createEntity(name, parentId),
  }));
  const openContextMenu = (event: React.MouseEvent, entity?: EditorEntity): void => {
    event.preventDefault();
    event.stopPropagation();
    if (entity) window.plutoEditor.selectEntity(entity.id);
    const items: PopupMenuItem[] = entity ? [
      { label: 'Create Child', disabled: running, children: createItems(entity.id) },
      { label: 'Duplicate', shortcut: 'Ctrl+D', disabled: running, action: () => window.plutoEditor.duplicateEntity(entity.id) },
      { label: 'Frame Selected', shortcut: 'F', action: () => window.plutoEditor.frameSelected() },
      { label: entity.active ? 'Disable' : 'Enable', disabled: running, action: () => window.plutoEditor.setEntityActive(entity.id, !entity.active) },
      ...(entity.parentId ? [{ label: 'Move to Root', disabled: running, action: () => window.plutoEditor.reparentEntity(entity.id, 0) } satisfies PopupMenuItem] : []),
      { label: 'Delete', shortcut: 'Del', disabled: running, danger: true, separatorBefore: true, action: () => window.plutoEditor.deleteEntity(entity.id) },
    ] : [
      { label: 'Create', disabled: running || !editor, children: createItems(0) },
    ];
  ...
  return <PanelFrame id="hierarchy" title="Hierarchy" actions={create} className="hierarchy-panel">
    ...
    <div className="hierarchy-root" onContextMenu={(event) => openContextMenu(event)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => {
      ...
      {roots.map((entity) => <HierarchyNode key={entity.id} entity={entity} entities={editor?.entities ?? []} selectedId={editor?.selectedEntityId ?? 0} disabled={running} openContextMenu={openContextMenu} />)}
