...
function PostProcessStackEditor({ effects, effectTypes, presetReference, disabled, onAdd, onRemove, onMove, onEnabled, onParameter, onPreset, onSavePreset, onSavePresetAs }: {
  ...
  onPreset(reference: string): void; onSavePreset(): void; onSavePresetAs(reference: string): void;
}): React.JSX.Element {
  ...
  return <div className="post-process-stack">
    <div className="preset-row"><input disabled={disabled} value={preset} placeholder="project://PostProcessing/MyPreset.plutopostprocess" onChange={(event) => setPreset(event.currentTarget.value)} /><button disabled={disabled || preset === presetReference} onClick={() => onPreset(preset)}>Load</button><button disabled={disabled || !presetReference} onClick={onSavePreset}>Save</button><button disabled={disabled} onClick={() => setSavingAs(true)}>Save As…</button><button disabled={disabled || (!preset && !presetReference)} onClick={() => { setPreset(''); onPreset(''); }}>Clear</button></div>
    ...
    {savingAs && <NameDialog title="Save Post Process Preset" confirmLabel="Save" onClose={() => setSavingAs(false)} onConfirm={(raw) => { const name = raw.replace(/[^a-zA-Z0-9_-]+/g, '_').replace(/^_+|_+$/g, ''); if (name) { onSavePresetAs(`project://PostProcessing/${name}.plutopostprocess`); setSavingAs(false); } }} />}
  </div>;
}
