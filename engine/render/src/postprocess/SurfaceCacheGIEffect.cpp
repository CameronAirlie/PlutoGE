    ...
    void SurfaceCacheGIEffect::Initialize()
    {
        ...
        gather.fragmentSource = R"(#version 330 core
            ...
            void main(){
                ...
                float screenHits=0.0;float voxelHits=0.0;float rotation=fract(hash13(floor(position*32.0))+float(uFrameIndex)*0.61803398875);vec3 origin=position+normal*0.04;
        ...
        indirectGather.fragmentSource = R"(#version 430 core
            ...
            float hash13(vec3 p){p=fract(p*.1031);p+=dot(p,p.yzx+33.33);return fract((p.x+p.y)*p.z);}
            vec3 rayDirection(vec3 p,vec3 n,int index){float u=fract((float(index)+.5)/float(max(uRayCount,1))+float(uFrameIndex)*.754877666);u=max(u,.02);float phi=6.2831853*fract(float(index)*.61803398875+hash13(floor(p*32.0))+float(uFrameIndex)*.569840296);float radius=sqrt(u);vec3 local=vec3(radius*cos(phi),radius*sin(phi),sqrt(max(1.0-u,0.0)));vec3 helper=abs(n.z)<.999?vec3(0,0,1):vec3(1,0,0);vec3 t=normalize(cross(helper,n)),b=cross(n,t);return normalize(t*local.x+b*local.y+n*local.z);}
            ...
            void main(){vec3 p=texture(uScenePositionTexture,UV).xyz,raw=texture(uSceneNormalTexture,UV).xyz;if(length(raw)<.01){FragColor=vec4(0);return;}vec3 n=normalize(raw),origin=p+n*.04,total=vec3(0);float resolved=0;for(int rayIndex=0;rayIndex<8;rayIndex++){if(rayIndex>=uRayCount)break;vec3 direction=rayDirection(p,n,rayIndex),hitP,hitN,radiance,voxelRadiance=vec3(0);bool screenHit=traceScreen(origin,direction,hitP,hitN);bool hit=screenHit;if(!hit)hit=traceVoxel(origin,direction,hitP,hitN,voxelRadiance);if(!hit)continue;bool cardResolved=sampleCard(hitP,hitN,screenHit,radiance);if(cardResolved){total+=radiance;resolved+=1.0;}else if(!screenHit&&dot(voxelRadiance,vec3(.2126,.7152,.0722))>.0001){const float voxelConfidence=.65;total+=min(voxelRadiance,vec3(8.0))*voxelConfidence;resolved+=voxelConfidence;}}float rayCount=float(max(uRayCount,1));vec3 estimate=total/rayCount;float luminance=dot(estimate,vec3(.2126,.7152,.0722));estimate*=min(1.0,4.0/max(luminance,.0001));FragColor=vec4(estimate,clamp(resolved/rayCount,0.0,1.0));}
        )";
        ...
        temporal.fragmentSource = R"(#version 330 core
            ...
            uniform mat4 uView,uPreviousView,uPreviousViewProjection;uniform int uHasHistory;uniform float uTemporalBlend;
            ...
            void main(){
                ...
                if(uHasHistory!=0){
                    vec4 previousClip=uPreviousViewProjection*vec4(p,1.0);
                    vec2 historyUv=previousClip.w>.0001?previousClip.xy/previousClip.w*.5+.5:UV-texture(uMotion,UV).xy;
                    vec2 motion=UV-historyUv;
                    if(all(greaterThanEqual(historyUv,vec2(0)))&&all(lessThanEqual(historyUv,vec2(1)))){
                        ...
                        if(foundHistory){
                            ...
                            float historyWeight=uTemporalBlend*(1.0-clamp(current.a,0.0,1.0)*.05)*(1.0-motionRejection);
