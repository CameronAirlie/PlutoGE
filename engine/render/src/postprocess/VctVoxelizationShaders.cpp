...
void main(){
 ...
 if(any(isnan(r))||any(isinf(r)))r=vec3(0);r=clamp(r,vec3(0),vec3(16))*radianceCoverage;uvec3 encoded=uvec3(round(r*4095.0));uint encodedOpacity=uint(round(opacity*4095.0));ivec3 coord=clamp(ivec3(tc*imageSize(uAccumulationR)),ivec3(0),imageSize(uAccumulationR)-ivec3(1));if(uCaptureSurface!=0 && encodedOpacity!=0u){
 uvec3 n=uvec3(round(clamp(normal*.5+.5,vec3(0),vec3(1))*31.0));
 uvec3 c=uvec3(round(clamp(a.rgb*(1.0-metallic),vec3(0),vec3(.95))*31.0));
 uint surfaceBits=0x80000000u|n.x|(n.y<<5)|(n.z<<10)|(c.r<<15)|(c.g<<20)|(c.b<<25);
...
layout(rgba16f,binding=6)readonly uniform image3D uSecondaryVolume;
uniform float uSecondaryGain;
