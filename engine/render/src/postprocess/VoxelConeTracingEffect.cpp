...
namespace PlutoGE::render
{
        ...
        for (const auto &p : parameters)
        {
            ...
            else if (p.name == "Volume Size")
            {
                ...
                if (next != m_volumeSize)
                {
                    ...
                    m_cacheOriginSize = glm::vec4(0.0f); m_probeSchedule.Reset();
                    ResetHistory();
                }
            }
            else if (p.name == "World Cache")
            {
                const bool next = ParseBool(p.value);
                if (next != m_worldCache) { m_worldCache = next; ReleaseVolume(); ResetHistory(); }
            }
            else if (p.name == "Cache Size")
            {
    ...
    {
        m_voxelizationShader = Shader::Create(detail::VctVoxelizationShaderSource());
        m_voxelResolveShader = Shader::Create(detail::VctResolveShaderSource());
...
uniform sampler3D uProbeRadiance,uProbeVisibility;uniform vec4 uCacheOriginSize;uniform float uCacheBlend;uniform int uWorldCache;
...
vec4 sampleWorld(vec3 p,vec3 d,float diameter,out float voxelSize){
 ...
 for(int c=0;c<3;c++){
  ...
  if(weight>0.0)result+=sampleCascadeDirectional(c,tc,d,lod)*weight;
  voxelSize+=weight*(localVoxel-uCascadeSize[max(uCascadeCount-1,0)]/resolution);remaining-=weight;
 }
 return result;
}
...
void main(){vec3 p=texture(uScenePositionTexture,UV).xyz,rawNormal=texture(uSceneNormalTexture,UV).xyz,surfaceTc;float normalLengthSquared=dot(rawNormal,rawNormal);int surfaceCascade=findCascade(p,surfaceTc);if(!(normalLengthSquared>=.1&&normalLengthSquared<=1e6)||surfaceCascade<0){FragColor=vec4(0);return;}vec3 n=rawNormal*inversesqrt(normalLengthSquared);float viewDepth=max(-(uView*vec4(p,1)).z,0.0);
 ...
 FragColor=vec4(total*coverageAt(p)*uIntensity/max(float(uConeCount),1.0),viewDepth);})";
        ...
        {
            ShaderSource update; update.computeSource = probeCode.glsl;
            m_probeUpdateShader = Shader::Create(update);
        }
...
void main(){vec3 p=texture(uScenePositionTexture,UV).xyz,rawNormal=texture(uSceneNormalTexture,UV).xyz;float normalLengthSquared=dot(rawNormal,rawNormal);if(!(normalLengthSquared>=.01&&normalLengthSquared<=1e6)){FragColor=vec4(0);return;}vec3 n=rawNormal*inversesqrt(normalLengthSquared);float d=max(-(uView*vec4(p,1)).z,0.0);FragColor=vec4(encodeNormal(n),d,1);})";
    ...
    void VoxelConeTracingEffect::EnsureResources(int width, int height)
    {
        ...
        if (m_allocatedResolution != m_resolution || m_allocatedCascadeCount != desiredCascadeCount)
        {
            ...
            for (auto *texture : {&m_probeRadiance, &m_probeVisibility})
            {
                glGenTextures(1, texture); Graphics::BindTexture(GL_TEXTURE_3D, *texture);
    ...
    RenderTarget *VoxelConeTracingEffect::GenerateResolvedIndirectLighting(const PostProcessContext &context, int width, int height)
    {
        ...
        const bool useCache = m_worldCache && m_probeUpdateShader;
        if (useCache && m_cacheOriginSize.w == 0.0f)
        {
            const float size = std::max(m_cacheSize, m_volumeSize * std::pow(3.0f, float(m_requestedCascadeCount - 1)));
            const float snap = size / 16.0f;
            m_cacheOriginSize = glm::vec4(glm::floor(cameraPosition / snap) * snap - glm::vec3(size * 0.5f), size);
        }
        ...
        for (std::size_t cascadeIndex = 0; cascadeIndex < m_activeCascadeCount; ++cascadeIndex)
        {
            auto &cascade = m_cascades[cascadeIndex];
            cascade.size = m_volumeSize * std::pow(3.0f, static_cast<float>(cascadeIndex));
            if (useCache && cascadeIndex + 1 == m_activeCascadeCount)
            {
                cascade.size = m_cacheOriginSize.w;
        ...
        if (activeRebuild < m_activeCascadeCount && VoxelizeChunk(activeRebuild, context))
        {
            ...
            if (useCache && activeRebuild + 1 == m_activeCascadeCount) m_probeSchedule.Refresh();
            // Depth/normal validation retains history for the same receiver even when a field moves.
        }
        ...
        if (availableCascadeCount == 0 || (useCache && availableCascadeCount < static_cast<int>(m_activeCascadeCount)))
            return nullptr;
        ...
        const auto bindTraceVolumes = [&](Shader *shader)
        {
            ...
            Graphics::ActiveTexture(GL_TEXTURE14); Graphics::BindTexture(GL_TEXTURE_3D, m_probeRadiance);
            Graphics::ActiveTexture(GL_TEXTURE15); Graphics::BindTexture(GL_TEXTURE_3D, m_probeVisibility);
            shader->SetUniform("uProbeRadiance", 14); shader->SetUniform("uProbeVisibility", 15);
            shader->SetUniform("uCacheOriginSize", m_cacheOriginSize);
            shader->SetUniform("uCacheBlend", useCache ? m_probeSchedule.blend : 0.0f);
            ...
            (m_probeSchedule.clear || m_probeSchedule.remaining > 0))
        {
            ...
            {
                Graphics::ActiveTexture(GL_TEXTURE7 + static_cast<unsigned>(direction));
                Graphics::BindTexture(GL_TEXTURE_3D, m_radianceAtlases[direction]);
            }
            glBindImageTexture(1, m_probeRadiance, 0, GL_TRUE, 0, GL_READ_WRITE, GL_RGBA16F);
            glBindImageTexture(2, m_probeVisibility, 0, GL_TRUE, 0, GL_READ_WRITE, GL_RGBA16F);
            const auto dispatch = [&](unsigned first, unsigned count, bool clear)
            {
