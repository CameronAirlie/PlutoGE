...
int main()
{
    ...
    const auto composite = graph.AddPass({.name = "Composite", .inputs = {{Semantic::SceneColor, scene}, {Semantic::Auxiliary0, blurred}}, .writes = {output}});
    const auto downsample = graph.AddPass({.name = "Downsample", .inputs = {{Semantic::SceneColor, bright}}, .writes = {blurred}});
    const auto prefilter = graph.AddPass({.name = "Prefilter", .inputs = {{Semantic::SceneColor, scene}}, .writes = {bright}});
    ...
    invalid.AddPass({.name = "Invalid", .inputs = {{Semantic::SceneColor, orphan}}, .writes = {invalidOutput}});
