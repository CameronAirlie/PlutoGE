...
int main()
{
    ...
    try
    {
        ...
        auto *roads = scene.AddEntity(std::make_unique<Entity>(EntityConfig{.name = "Roads"}));
        auto *roadGroup = scene.AddEntity(std::make_unique<Entity>(EntityConfig{.name = "Road Group"}), roads);
        junction->SetParent(roadGroup);
        roads->AddTag("ground");
        Check(scene.RaycastByTag({0, 3, 22.5f}, {0, -1, 0}, 10, "ground", junctionHit) && junctionHit.entityId == junction->GetID(), "Road did not inherit ancestor ground tag");
        Check(junction->GetTags().empty(), "Inherited raycast tag was copied onto road");
        roads->RemoveTag("ground");
        Check(!scene.RaycastByTag({0, 3, 22.5f}, {0, -1, 0}, 10, "ground", junctionHit), "Removed ancestor tag still matched");
        junction->AddTag("ground");
        Check(scene.RaycastByTag({0, 3, 22.5f}, {0, -1, 0}, 10, "ground", junctionHit), "Local road tag stopped matching");
        junction->RemoveTag("ground");
        roads->AddTag("ground");
        junction->SetParent(nullptr);
        Check(!scene.RaycastByTag({0, 3, 22.5f}, {0, -1, 0}, 10, "ground", junctionHit), "Reparented road retained ancestor tag");
